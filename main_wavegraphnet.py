"""
WaveGraphNet – mirrors the coupling-models.ipynb training exactly:

Phase 1 (--fwd_epochs):  Pre-train forward model only, inverse frozen.
Phase 2 (--epochs):      Train inverse only.
                         Forward is FROZEN (eval mode, not in optimizer).
                         Loss = L_inv + lambda(epoch) * L_fwd
                         lambda linearly anneals 0 → max_lambda after warmup.
                         Scheduler steps on val MAE (Euclidean, mm), not train MSE.
"""
...
from utils.precompute import build_all_stats, N_ATTENTION_FREQS, LOOKBACK_POINTS
...
def get_lambda(epoch, total, warmup, max_lam):
    """Linear annealing from 0 → max_lam after warmup (matches notebook Cell 44)."""
    if epoch < warmup:
        return 0.0
    dur = total - warmup
...
def train_phase2(inv_model, fwd_model, loader, opt_inv, lam, prop_ei, device):
    """
    Matches notebook Cell 50 exactly:
    - inv_model: train mode
    - fwd_model: eval mode (frozen — NOT in optimizer)
    - optimizer contains ONLY inv_model parameters
    """
    inv_model.train()
    fwd_model.eval()   # ← frozen forward
    crit = nn.MSELoss()
    tot_inv = tot_fwd = 0.0
...
# ── Evaluate: Euclidean MAE (matches notebook Cell 47) ──────────────────────
def _euclidean_mae(model, loader, device):
    """Returns mean Euclidean distance (normalized coords) over damaged samples."""
    model.eval()
    total = 0.0; count = 0
    with torch.no_grad():
        for batch in loader:
...
def main():
    ...
    # ── Build all stats from TRAINING split (time-domain + freq-domain) ────────
    stats = build_all_stats(raw_data_map, train_ids)
    norm_data = stats["normalized_data_map"]   # use THIS, not raw_data_map
    ...
    if args.mode == "coupled" and args.fwd_epochs > 0:
        ...
        for ep in range(1, args.fwd_epochs + 1):
            ...
            # evaluate on val using inv (untrained, just for logging)
            vm, vmm = _euclidean_mae(inv_model, vl, device), float("nan")
            sch_f.step(fl)
            if ep % args.val_every == 0 or ep in (1, args.fwd_epochs):
                print(f"  [Fwd] Ep {ep:04d} | FwdLoss={fl:.6f} "
                      f"| LR={opt_f.param_groups[0]['lr']:.2e}", flush=True)
            if fl < best_fwd_loss:
                best_fwd_loss = fl
                torch.save(fwd_model.state_dict(), best_fwd_path)

        print(f"  Loading best forward model (loss={best_fwd_loss:.6f})")
    ...
    # ONLY inv_model params in optimizer — forward is frozen
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % args.val_every == 0 or epoch in (1, args.epochs):
            # Euclidean MAE (matches notebook evaluate())
            val_mae_norm  = _euclidean_mae(inv_model, vl, device)
            val_mae_mm    = val_mae_norm * PLATE_MM
            # Step scheduler on val MAE (matches notebook Cell 57: scheduler.step(eval_mae))
            ...
            if val_mae_mm < best_val_mae:
                ...
                save_checkpoint(ckpt_path, config=vars(args), test_loss=test_mse,
                                val_loss=val_mae_mm,
